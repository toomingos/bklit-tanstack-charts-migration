...
**Impact on this file (01 — bklit-ui inventory):** none on the inventory body — `repos/bklit-ui` was not modified post-freeze. D144 confirms bklit's native `@visx/zoom <Zoom>` is the API the migrated choropleth now mirrors (relevant to row 14's "confirmed `@visx/zoom`" note).
