    ...
    const tick = () => {
      ...
      if (chrome) {
        ...
        if (cursorX === null) {
          ...
          const visible = dataRef.current.filter((p) => p.time >= windowStartSec);
          visible.push({ time: next.now / 1000, value: next.displayValue });
          visible.push({ time: (next.now + xTickUnitMs) / 1000, value: next.displayValue });
          ...
          if (val === null) {
            ...
            const chromeMargin = chromeConfigRef.current.margin;
            const py = (yScaleNext(val) ?? 0) + chromeMargin.top;
            const formatTime = liveXAxisRef.current?.formatTime ?? defaultFormatTime;
            chrome.updateHover({
              point: {
                x: cursorX + chromeMargin.left,
      ...
      const shouldCommit =
        !pausedRef.current ||
        pixelChange >= PAUSED_FRAME_PIXEL_THRESHOLD;
      ...
      // Once paused convergence is below the visual threshold, the loop has
      // no rendered work left. The effect is restarted when `paused` or the
      // target range changes, so resuming or receiving new data is lossless.
      if (pausedRef.current && pixelChange < PAUSED_FRAME_PIXEL_THRESHOLD) return;
      wakeLoopRef.current?.();
    };
    wakeLoopRef.current = () => {
