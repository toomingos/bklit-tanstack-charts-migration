...
export function createRingHoverRuntime(): RingHoverRuntime {
  ...
  const applyTransform = () => {
    console.log("[ring-hover] applyTransform, started:", started, "config:", !!config);
  ...
  return {
    ...
    paint(hoveredIndex) {
      console.log("[ring-hover] paint, hovered:", hoveredIndex, "started:", started);
      if (!config) return;
    ...
    settleAtRest() {
      console.log("[ring-hover] settleAtRest called, started:", started);
      if (started) return;
      started = true;
