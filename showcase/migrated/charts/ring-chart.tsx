...
const ringCleanupMap = new WeakMap<Element, () => void>();
...
export function RingChart({
  ...
  // -----------------------------------------------------------------------
  // handleRender — WAAPI reveal ONLY. Runs once (guarded by DOM attribute on
  // the SVG element). No spring creation, no pointer listeners, no
  // subscriptions. The hover chrome is set up separately in the
  // useLayoutEffect below.
  // -----------------------------------------------------------------------
  const ringElementMapRef = useRef<{
    trackGroups: Map<number, SVGGElement>;
    progressGroups: Map<number, SVGGElement>;
    progressPaths: Map<number, SVGPathElement>;
  }>({ trackGroups: new Map(), progressGroups: new Map(), progressPaths: new Map() });
  ...
  const handleRender = useCallback(({ container }: { container: HTMLElement }) => {
    ...
    if (!svg || svg.dataset.bkmRevealed === "1") return;
    ...
    const trackGroups = new Map<number, SVGGElement>();
    const progressGroups = new Map<number, SVGGElement>();
    const progressPaths = new Map<number, SVGPathElement>();
    // Exact CSS attr selector lookup — the id values "ring-N-track" /
    // "ring-N-progress" are hardcoded in the mark definitions (lines ~437/451),
    // so this is NOT fragile to TanStack internal format changes.
    for (let i = 0; i < currData.length; i++) {
      const trackGroup = svg.querySelector(`[data-ts-key="ring-${i}-track"]`);
      const progressGroup = svg.querySelector(`[data-ts-key="ring-${i}-progress"]`);
      if (trackGroup) trackGroups.set(i, trackGroup as SVGGElement);
      if (progressGroup) {
        progressGroups.set(i, progressGroup as SVGGElement);
        const path = progressGroup.querySelector("path");
        if (path) progressPaths.set(i, path as SVGPathElement);
      }
    }
    ringElementMapRef.current = { trackGroups, progressGroups, progressPaths };
    ...
    for (let i = 0; i < currData.length; i++) {
      ...
      const trackGroup = trackGroups.get(i) ?? null;
      const progressGroup = progressGroups.get(i) ?? null;
      ...
      const progressPathEl = progressPaths.get(i) ?? null;
  ...
  useLayoutEffect(() => {
    ...
    for (const state of stateMap.values()) {
      state.runtime.stop();
      const cleanup = state.groupEl ? ringCleanupMap.get(state.groupEl) : undefined;
      if (cleanup) {
        cleanup();
        ringCleanupMap.delete(state.groupEl!);
      }
    }
    stateMap.clear();
    ...
    const { trackGroups, progressGroups, progressPaths } = ringElementMapRef.current;
  ...
  // the rare case where the effect hasn't run (e.g., geometryScrubbing is
  // true or the SVG isn't in the DOM yet).
  useEffect(() => {
    return () => {
      const stateMap = ringStateRef.current;
      for (const state of stateMap.values()) {
