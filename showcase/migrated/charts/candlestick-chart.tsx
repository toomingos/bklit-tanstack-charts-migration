...
// replay the reveal.
//
// Hover chrome: native pointermove/bisect listener (scatter's pattern,
// `bisectDateLeft`, strict `>` tie-break) driving `candlestick-hover-chrome.ts`
// — bklit's own `resolveTooltipFromX` forward-maps the candle's OWN center
// through the SAME scale instances the marks render through (stashed via the
// x/y `ChartScale` `resolve()` hatch — D18: never stash a pre-resolve scale).
...
export function CandlestickChart({
  ...
  // Stashed ranged d3 scale instances (D18: only ever stash from INSIDE a
  // `ChartScale.resolve()` closure, never a pre-resolve instance) — used by
  // both the native hover listener and the locally-recomputed Y-axis ticks.
  ...
  // Augmented data rows with original index — the `__dataIdx` field feeds
  // `link()`'s `key` accessor so each DOM `<line>` carries its source row
  // index, used by the reveal animation's stagger math even across filtered
  // marks (gains/losses are disjoint subsets of the full row order).
  const indexedRows = React.useMemo(
    () => renderData.map((d, i) => ({ ...d, __dataIdx: i })),
    [renderData],
  );
  ...
  const definition = React.useMemo(() => {
    ...
    const wicksMark = createMark(() => {
      const xValues = indexedRows.map((d: any) => d[xDataKey] as Date);
      const lowValues = indexedRows.map((d: any) => d.low as number | undefined);
      const highValues = indexedRows.map((d: any) => d.high as number | undefined);
      return {
        id: "wicks",
        channels: {
    ...
    const bodiesMark = createMark(() => {
      const xValues = indexedRows.map((d: any) => d[xDataKey] as Date);
      const openValues = indexedRows.map((d: any) => d.open as number | undefined);
      const closeValues = indexedRows.map((d: any) => d.close as number | undefined);
      return {
        id: "bodies",
        channels: {
          x: { scale: "x", values: xValues },
  ...
  // Native pointermove/pointerleave hover (bypasses TanStack's own
  // `focus:"group-x"` resolution) — mirrors bklit's `resolveTooltipFromX`
  // exactly: forward-maps the resolved candle's OWN center date/close value
  // through the SAME stashed scale instances the marks render through.
  const hoverInputsRef = React.useRef({ data: renderData, xDataKey, bodyWidthPx, resolvedCandlestick });
  hoverInputsRef.current = { data: renderData, xDataKey, bodyWidthPx, resolvedCandlestick };
  ...
  React.useLayoutEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      const { data, xDataKey: key, bodyWidthPx: bw, resolvedCandlestick: rc } =
      ...
      const open = datum.open as number;
      const close = datum.close as number;
      const low = datum.low as number;
      const high = datum.high as number;
      const fill = close >= open ? rc.positiveFill : rc.negativeFill;
      const centerXpx = xScaleInstance(datumX) ?? 0;
      const yOpen = yScaleInstance(open) ?? 0;
      const yClose = yScaleInstance(close) ?? 0;
      const yLow = yScaleInstance(low) ?? 0;
      const yHigh = yScaleInstance(high) ?? 0;
  ...
  const handleRender = React.useCallback(() => {
    ...
    onPostPaint(() => {
          ...
          // Extract the original data index from `data-ts-key`. With our
          // custom mark keys ("wicks:N" / "bodies:N"), the last segment
          // after ":" is the 0-based data index.
          const parseIdx = (el: Element): number => {
            const key = el.getAttribute("data-ts-key") ?? "";
            const lastColon = key.lastIndexOf(":");
            const lastDash = key.lastIndexOf("-");
            const sep = Math.max(lastColon, lastDash);
            return sep >= 0 ? Number(key.slice(sep + 1)) : 0;
          };

          const staggerBaseMs = n > 0 ? (animationDuration * 0.6) / n : 0;

          const useCssRevealFastPath = enterBounce === DEFAULT_ENTER_BOUNCE;
          ...
          // Wicks cover all rows in order — DOM index = data index.
          wickRects.forEach((rect, i) => applyReveal(rect, i));
          // Bodies cover all rows in order with their parsed index.
          bodyRects.forEach((rect) => applyReveal(rect, parseIdx(rect)));
          marksGroup.classList.remove("ts-chart__marks--revealing");
