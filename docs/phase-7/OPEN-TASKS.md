...
| **liveline/100 bklit tail** | `virtual-settle-tail>700ms` flagged on the **bklit** side at `--repeats 5` having never flagged at 3 | Not a tail: raw `[123,120,123,135,1303]` — four readings within 15 ms and one slow frame. The predicate was `ok.some(r > 700)` on a row where every other statistic is a median, so its false-alarm rate grew with `--repeats` while the cell did not | **fixed (D643).** Predicate is now the median of the same values, which is the number the row already prints. Verified at `--repeats 5`: flags 3 → 2, liveline clears, and D641's `bar/100` (`[1092,1091,1133,1101,1069]`, median 1092) still flags |
