...
| G15 | lead (V3.2 sankey review) | sankey hover dim is owned twice: `withStates` on link/rect (V3.2) and the reactive definition rebuild kept from V2.2 because D528(a) said states could not attach; D528(a) no longer reproduces with package `link`/`rect` marks | V2 (pointer) | V2.2 sankey follow-up row: delete the reactive dim path, states own dim; hover cells stay the proof | D536 |
