...
| 3 | V2.2 polar | pie · ring · sunburst · radar · gauge: package pointer/focus, `focusGroupAngle`, delete `polar-hit.ts`, `pie-hover-chrome.ts`, `sunburst-hit*.tsx`, `focusDisabled`, radar listeners, `styles.css:193` | merged | `ses_f8d4d765affeN3G7OH71ZjWuOO` | (next commit) | D535 · hover-invariant definitions, states carry hover · 5 modules deleted · sunburst + radar `ChartFocusStrategy` · radar/6 hover-50 ruled 7000 (1.05 scale gap) · lint floor 7 → 4 |
...
| 3 | V3.2 sankey | `sankeyDiagram` with package `rect`/`text` marks, declared gradients, `d3-sankey`/`createElementNS` = 0 | dispatched | | | after V2.2 sankey (`d3de477`); one measured `withStates` attempt (D528) |
