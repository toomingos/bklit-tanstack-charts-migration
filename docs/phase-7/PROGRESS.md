...
| 3 | V2.2 | package pointer and focus — pie · ring · sunburst · gauge · radar · composed (one row per family when dispatched) | todo | | | polar families wait for G7 (`internal/gauge-*` and `pie-hover-chrome.ts` importers); composed waits for V1.2 |
