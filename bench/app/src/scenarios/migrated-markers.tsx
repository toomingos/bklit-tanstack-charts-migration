// Same composition as bklit-markers.tsx; only the ChartMarkers import source differs.
...
function buildMarkers(rows: SeededRow[]): ChartMarker[] {
  ...
  return [
    // >=3 same-date markers take the fan-out badge path.
...
// GUARD: settle margin = animationDuration(1100) + dot fade(500) + 2 frames; change with line-chart.
// GUARD: migrated-only (bklit dots complete at +0); honest capture is post-reveal, not pre-animation.
...
export default function MigratedMarkers({ n }: { n: number }) {
  ...
  // Manual settle: shared 2500ms fallback would fire before ready + reveal tail.
  const { resolve: resolveSettled } = useMemo(
    () => armManualSettle(MARKER_REVEAL_TAIL_MS + 3000),
    [],
  );
  // A ready with no preceding non-ready phase is not a completed reveal.
