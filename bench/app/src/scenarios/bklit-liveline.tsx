// Docs-demo port (registry props are broken); seeded data replaces the Math.random() hook.
// Freeze: push 10 ticks at real 600ms cadence (wall-clock x geometry), wait 2s y-lerp, pause, resolve.
...
// Fallback is safety-net only: sized with headroom over the ~8s freeze duration.
...
export default function BklitLiveLine({
  ...
  useEffect(() => {
    ...
    if (scenario === "live") {
      // No freeze ticks: wait out the y-lerp, resolve, leave the rAF loop running.
      (async () => {
    ...
    // Freeze: K ticks at real cadence, wait y-lerp, pause, resolve.
    let count = 0;
